...
class ESPnetS2TModel(AbsESPnetModel):
    ...
    def __init__(
        ...
        self.mask_speech_type = mask_speech_type
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Add mask_frame check
        if mask_frame is not None:
            assert mask_frame.shape[0] == speech.shape[0], (
                mask_frame.shape,
                speech.shape,
            )
        batch_size, length = speech.shape[:2]
        if self.training:
            if self.mask_speech_type == "random":
                mask_length = int(length * self.mask_ratio)
                if length > mask_length:
                    start_points = torch.randint(0, length - mask_length, (batch_size,))
                    mask = torch.arange(length).expand(batch_size, length)
                    mask = (mask >= start_points[:, None]) & (mask < (start_points[:, None] + mask_length))
                    speech[mask] = 0
            elif self.mask_speech_type == "word" and mask_frame is not None:
                start_points = mask_frame[:, 0]
                end_points = mask_frame[:, 1]
                end_points = torch.clamp(end_points, max=length)
                mask = torch.arange(length).expand(batch_size, length).to(speech.device)
                mask = (mask >= start_points[:, None]) & (mask < end_points[:, None])
                speech[mask] = 0
        ...
        encoder_out, encoder_out_lens = self.encode(speech, speech_lengths, _clip_feature)
    ...
    def encode(
        ...
        if self.encoder.interctc_use_conditioning:
            ...
            encoder_out, encoder_out_lens, _ = self.encoder(feats, feats_lengths, clip_feature)
        intermediate_outs = None
        if isinstance(encoder_out, tuple):
            intermediate_outs = encoder_out[1]
            encoder_out = encoder_out[0]
    ...
    def _calc_att_loss(
        ...
        # for enc_out, enc_out_len  in zip(encoder_out, encoder_out_lens):
        #     if self.num_frames > 1:
        #         enc_out = enc_out[self.num_frames:]
        #         enc_out_len = len(enc_out)
        #     _encoder_out.append(torch.cat(enc_out))
        #     _encoder_out_lens.append(enc_out_len)
        # _encoder_out = torch.tensor(_encoder_out)
        # _encoder_out_lens = torch.tensor(_encoder_out_lens).to(_encoder_out_lens)
